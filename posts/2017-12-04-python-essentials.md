+++
layout = "post"
title = "Beautiful and idiomatic Python with Raymond Hattinger"
categories = ["python"]
tags = ["python"]
description = "Beautiful and idiomatic Python with Raymond Hattinger"
+++

All credits to
[JeffPaine](https://github.com/JeffPaine/beautiful_idiomatic_python), who
summarized this incredibly useful
[talk of the infamous Raymond Hattinger](https://www.youtube.com/watch?v=OSGv2VnC0go&).
All of the following content is copied from his
[public repository on GitHub](https://github.com/JeffPaine/beautiful_idiomatic_python):
...
The code examples and direct quotes are all from Raymond's talk. I've reproduced
them here for my own edification and the hopes that others will find them as
handy as I have!
...
This approach is much more memory efficient than `range`. `xrange` was renamed
to `range` in python 3.
...
`zip` creates a new list in memory and takes more memory. `izip` is more
efficient than `zip`. Note: in python 3 `izip` was renamed to `zip` and promoted
to a builtin replacing the old `zip`.
...
The original is slow and unpleasant to write. Also, comparison functions are no
...
When should you use the second and not the first? When you're mutating the
dictionary.

> If you mutate something while you're iterating over it, you're living in a
> state of sin and deserve what ever happens to you.

`d.keys()` makes a copy of all the keys and stores them in a list. Then you can
modify the dictionary. Note: in python 3 to iterate through a dictionary you
have to explicitly write: `list(d.keys())` because `d.keys()` returns a
"dictionary view" (an iterable that provide a dynamic view on the dictionary’s
keys). See
...
`iteritems()` is better as it returns an iterator. Note: in python 3 there is no
`iteritems()` and `items()` behaviour is close to what `iteritems()` had. See
[documentation](https://docs.python.org/3/library/stdtypes.html#dict-views).
...
`popitem` is atomic so you don't have to put locks around it to use it in
threads.
...
Is slightly (microseconds) slower but is worth it for the code clarity and
...
A namedTuple is a subclass of tuple so they still work like a regular tuple, but
are more friendly.
...
- x and y are state, and state should be updated all at once or in between lines
  that state is mis-matched and a common source of issues
- ordering matters
- it's too low level

The second approach is more high-level, doesn't risk getting the order wrong and
is fast.
...
Note: since python 3.2 there is a decorator for this in the standard library:
`functools.lru_cache`.
...
`ignored` is is new in python 3.4,
[documentation](http://docs.python.org/dev/library/contextlib.html#contextlib.ignored).
...
`redirect_stdout` is proposed for python 3.4,
[bug report](http://bugs.python.org/issue15805).
