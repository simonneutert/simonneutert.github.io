...
Yet, there is more to that. You can call `.size` to gain a little more speed,
...
If you need the objects return by a query. Make sure to run your element query
first, then use `size` to not make the database go through the rows again -
using `count` will hit the database.
[postgresqltutorial.com: COUNT function](http://www.postgresqltutorial.com/postgresql-count-function/)
...
Once your platform is growing big, you could either setup a rake task that
updates a reference table frequently and/or make use of rails' built-in
[counter_cache](http://guides.rubyonrails.org/association_basics.html#belongs-to-association-reference#4.1.2.3)
